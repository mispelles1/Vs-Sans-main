...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		{
			var menuItem:FlxSprite = new FlxSprite(0, 10 + (i * 150));
			menuItem.frames = tex;
			menuItem.animation.addByPrefix('idle', optionShit[i] + " basic", 24);
			menuItem.animation.addByPrefix('selected', optionShit[i] + " white", 24);
			menuItem.animation.play('idle');
			menuItem.ID = i;
			FlxTween.tween(menuItem, { x: menuItem.width/3 + (i * 10) - 0}, 1.7, { ease: FlxEase.expoInOut });
		...
		if (!selectedSomethin)
		{
			...
			if (controls.ACCEPT)
			{
				if (optionShit[curSelected] == 'donate')
				{
					...
					DiscordClient.changePresence("Goin' To Read The Credits!", null);
				}
				else
				{
